...
/**
 * @brief inicialization of bluetooth module 
 */
...
/**
 * @brief offset of the direction encoder; which reading is corresponding to moving in straignt line
 */
...
/**
 * @brief time when was the last message sent 
 */
float timeOfLastMessage = 0;
...
void set_dircZero() {
  /**
   * @brief basically distance for which you should move in straight line
   */
  int count = 1000;
  long int sum_dirc_reading = 0;
  int old_dist_position = 0;
  int reading_count = 0;
  while (abs(dist_position) < count)
  {
    if( dist_position != old_dist_position ) {
      sum_dirc_reading += dirc_position;
      reading_count++;
  ...
  dirc_position_offset = sum_dirc_reading / reading_count; 
  dist_position = 0;
...
/**
 * @brief Setup the Bluetooth object
 * 
 */
...
void create_NMEA(int originTime, float distance, float inclination) {
  ...
  float speed = distance / (millis() - timeOfLastMessage) * 1000.0;
  timeOfLastMessage = millis();
  int currentTime = (timeOfLastMessage - originTime) / 1000;
  String NMEA_distance = String(distance * 1000, 1);
  while (inclination < 0)   {
    inclination += 2 * M_PI;
  }
  while (inclination > 2 * M_PI)   {
    inclination -= 2 * M_PI;
  }  
  String NMEA_inclination = String(inclination, 4);
  String NMEA_speed = String(speed, 2);
  sprintf(NMEA_inside, "%s,%d,%s,%s,%s", 
                        NMEA_identificator,
                        currentTime, 
                        NMEA_distance.c_str(),
//                        distance >= 0 ? 'P' : 'N',
